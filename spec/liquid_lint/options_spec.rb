...
describe LiquidLint::Options do
  describe '#parse' do
    ...
    context 'with a list of files to exclude' do
      let(:args) { %w[--exclude some-glob-pattern/*.liquid,some-other-pattern.liquid] }

      it 'sets the `excluded_files` option to that list of patterns' do
        subject.should include excluded_files: %w[some-glob-pattern/*.liquid some-other-pattern.liquid]
      end
    end
    ...
    context 'with input from stdin' do
      let(:args) { %w[--stdin-file-path file1.liquid] }

      it 'sets the `stdin_file_path` option to passed file name' do
        subject.should include stdin_file_path: 'file1.liquid'
      end
    end
    ...
    context 'with a reporter option' do
      context 'for a reporter that exists' do
        ...
        it 'sets the `reporter` option' do
          subject.should include reporter: LiquidLint::Reporter::JsonReporter
        end
      end
      ...
      context 'for a reporter that does not exist' do
        ...
        it 'raises an error' do
          expect { subject }.to raise_error LiquidLint::Exceptions::InvalidCLIOption
        end
      end
    end
    ...
    context 'with a list of file glob patterns' do
      let(:args) { %w[app/**/*.liquid some-dir/some-template.liquid] }
