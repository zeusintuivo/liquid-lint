...
describe LiquidLint::Filters::InjectLineNumbers do
  describe '#call' do
    subject { described_class.new.call(sexp) }

    let(:sexp) { LiquidLint::Sexp.new([:one, [:newline], [:two, [:newline], :three]]) }
    ...
    context 'when a Sexp has atoms spanning multiple lines' do
      let(:sexp) { LiquidLint::Sexp.new(["one\ntwo", [:newline], ["three\nfour", [:newline], 5]]) }
    ...
    context 'when a Sexp has atoms with \n in front' do
      let(:sexp) do
        LiquidLint::Sexp.new([:one, [:newline], ["\ntwo", [:newline], ["\nthree\nfour", 5]]])
      end
