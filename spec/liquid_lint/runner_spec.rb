...
describe LiquidLint::Runner do
  ...
  describe '#run' do
    let(:files) { %w[file1.liquid file2.liquid] }
    ...
    context 'when :config_file option is specified' do
      ...
      it 'loads that specified configuration file' do
        ...
        LiquidLint::ConfigurationLoader.should_receive(:load_file)
    ...
    context 'when `exclude` global config option specifies a list of patterns' do
      ...
      let(:config) { LiquidLint::Configuration.new(config_hash) }
      let(:config_hash) { { 'exclude' => 'exclude-this-file.liquid' } }
      ...
      it 'passes the global exclude patterns to the FileFinder' do
        LiquidLint::FileFinder.any_instance
                            .should_receive(:find)
                            .with(files, ['exclude-this-file.liquid'])
                            .and_return([])
        subject
      end
    end
    ...
    context 'when `--stdin-file-path` option specified' do
      let(:options) { { stdin_file_path: 'file1.liquid' } }
