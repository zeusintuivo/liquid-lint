...
describe LiquidLint::Linter do
  ...
  describe '#run' do
    ...
    let(:document) { double(sexp: LiquidLint::Sexp.new(sexp), file: 'file.liquid', source_lines: []) }
  ...
  describe '#name' do
    ...
    before do
      linter.class.stub(:name).and_return('LiquidLint::Linter::SomeLinterName')
    end
