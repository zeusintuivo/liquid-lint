...
describe LiquidLint::ConfigurationLoader do
  describe '.load_applicable_config' do
    ...
    context 'when directory contains a configuration file' do
      ...
      around do |example|
        directory do
          File.open('.liquid-lint.yml', 'w') { |f| f.write(config_contents) }
          example.run
        end
      end
      ...
      it 'loads the file' do
        described_class.should_receive(:load_file)
                       .with(File.expand_path('.liquid-lint.yml'))
        subject
      end
  ...
  describe '.default_configuration' do
    ...
    it 'loads the default config file' do
      described_class.should_receive(:load_from_file)
                     .with(LiquidLint::ConfigurationLoader::DEFAULT_CONFIG_PATH)
      subject
    end
  end
    ...
    context 'with a file that exists' do
      ...
      context 'and is valid' do
        ...
        it 'extends the default configuration' do
          custom_config = LiquidLint::Configuration.new('skip_frontmatter' => true)
      ...
      context 'and is invalid' do
        ...
        it 'raises an error' do
          expect { subject }.to raise_error LiquidLint::Exceptions::ConfigurationError
        end
      end
    end
  ...
  describe '.load_hash' do
    ...
    context 'when hash is not empty' do
      ...
      it 'extends the default configuration' do
        subject.should == described_class.default_configuration
                                         .merge(LiquidLint::Configuration.new(hash))
      end
    end
  end
end
