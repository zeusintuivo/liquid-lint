...
describe LiquidLint::LinterSelector do
  let(:options) { {} }
  let(:config) { LiquidLint::ConfigurationLoader.load_hash(config_hash) }
  ...
  class FakeLinter1 < LiquidLint::Linter; include LiquidLint::LinterRegistry; end
  class FakeLinter2 < LiquidLint::Linter; include LiquidLint::LinterRegistry; end
  class FakeLinter3 < LiquidLint::Linter; include LiquidLint::LinterRegistry; end

  before do
    LiquidLint::LinterRegistry.stub(:linters)
                            .and_return([FakeLinter1, FakeLinter2, FakeLinter3])
  end

  describe '#linters_for_file' do
    let(:file) { 'some-file.liquid' }
    ...
    context 'when excluded_linters option specifies all included_linters' do
      ...
      it 'raises an error' do
        expect { subject }.to raise_error LiquidLint::Exceptions::NoLintersError
      end
    end
    ...
    context 'when linter specifies `include`/`exclude` in its configuration' do
      ...
      context 'and the file matches the include pattern' do
        let(:include_pattern) { '**/some-*.liquid' }
      ...
      context 'and the file does not match the include pattern' do
        let(:include_pattern) { '**/nope-*.liquid' }
      ...
      context 'and the file matches the exclude pattern' do
        let(:exclude_pattern) { '**/some-*.liquid' }
      ...
      context 'and the file matches the exclude pattern and the file is absolute path' do
        let(:file) { File.expand_path('some-file.liquid') }
        let(:exclude_pattern) { 'some-*.liquid' }
      ...
      context 'and the file matches both the include and exclude patterns' do
        let(:include_pattern) { '**/*-file.liquid' }
        let(:exclude_pattern) { '**/some-*.liquid' }
